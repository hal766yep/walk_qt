...
void vHeatMap::setData(std::vector<double> &channels)
{
    ...
    // if (++frameCount % 10 == 0) {  // 每10帧保存一次
    //     QString filename = QString("heatmap_frame_%1.png").arg(frameCount);
    //     autoSavePng(filename);
    // }
}
 ...
 * 先用80 * 80的网格，，然后放大至240 * 240，计算量为80 * 80 * 64 + 240 *240
 */
...
// vHeatMap::precompute , 提前计算，以减少复杂度
        ...
        if (startKeepTimestamp != 0)
        {
            ...
            if (foots.size() >= 2)
            {
                    ...
                    foots[foots.size() - 1]->Timestamp.front())  // 判定为一次
                {
                    ...
                    // 计算步频
                    double cadence = 0;
                    if (step_gap != 0) cadence = 60000 / step_gap;
                    ui->label_stepFrequency->setText(QString("%1 steps/min").arg(QString::number(cadence, 'f', 2)));
